...
/**
 * Nome do blob no Azure para vídeos DTP: video-{uuid}-{stem}{ext}
 */
export function buildAzureVideoBlobName(originalFileName: string, uniqueId: string): string {
  ...
  return `video-${uniqueId}-${stem}${ext}`.toLowerCase();
}

export function buildAzureDtpPdfBlobName(jobId: string): string {
  return `dtp-${jobId}.pdf`.toLowerCase();
}

export function buildAzureDtpScreenshotBlobName(jobId: string, order: number): string {
  return `dtp-screenshots/${jobId}/${order}.png`.toLowerCase();
}
